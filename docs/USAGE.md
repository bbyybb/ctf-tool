...
### Q: Antivirus flags the tool as malware?
### Q: 杀毒软件提示有恶意软件？
A: This is a **false positive**. As a CTF security tool, the source code contains attack payload strings (SQLi, XSS, webshell templates, etc.) for reference only — they are never executed. Add the project directory to your antivirus exclusion list. This is the same reason sqlmap, Metasploit, and Burp Suite trigger alerts.

A: 这是**误报**。作为 CTF 安全工具，源码中包含攻击载荷字符串（SQL 注入、XSS、Webshell 模板等）仅作参考，不会被执行。将项目目录加入杀毒软件白名单即可。sqlmap、Metasploit 等工具也会遇到同样问题。
