...
class ReverseModule:
    ...
    def tool_cheatsheet(self, input: str = "") -> str:
        """逆向工程工具命令速查表"""
        lines = [f"=== {t('rev.tool_cheatsheet')} ===", ""]

        lines.append("[1] GDB (GNU Debugger)")
        lines.append("  gdb ./binary                               # Start debugging")
        lines.append("  (gdb) info functions                       # List functions")
        lines.append("  (gdb) disas main                           # Disassemble main")
        lines.append("  (gdb) b *0x401234                          # Breakpoint at address")
        lines.append("  (gdb) b main                               # Breakpoint at function")
        lines.append("  (gdb) r                                    # Run")
        lines.append("  (gdb) r < input.txt                        # Run with input")
        lines.append("  (gdb) ni / si                              # Next/step instruction")
        lines.append("  (gdb) c                                    # Continue")
        lines.append("  (gdb) x/20x $rsp                          # Examine stack (hex)")
        lines.append("  (gdb) x/s 0x401234                        # Examine string")
        lines.append("  (gdb) info registers                       # All registers")
        lines.append("  (gdb) p $rax                               # Print register")
        lines.append("  (gdb) set $rax = 0x1234                    # Set register")
        lines.append("  (gdb) vmmap                                # Memory map (pwndbg)")
        lines.append("  (gdb) checksec                             # Security check (pwndbg)")
        lines.append("  (gdb) cyclic 200 / cyclic -l 0x61616161   # Pattern (pwndbg)")

        lines.append("\n[2] GDB Plugins")
        lines.append("  # pwndbg:  git clone https://github.com/pwndbg/pwndbg")
        lines.append("  # peda:    git clone https://github.com/longld/peda")
        lines.append("  # gef:     bash -c \"$(curl -fsSL https://gef.blah.cat/sh)\"")

        lines.append("\n[3] objdump / readelf")
        lines.append("  objdump -d binary                          # Disassemble all")
        lines.append("  objdump -d -M intel binary                 # Intel syntax")
        lines.append("  objdump -t binary                          # Symbol table")
        lines.append("  objdump -j .got.plt -d binary              # GOT entries")
        lines.append("  readelf -h binary                          # ELF header")
        lines.append("  readelf -S binary                          # Section headers")
        lines.append("  readelf -s binary                          # Symbol table")
        lines.append("  readelf -l binary                          # Program headers")
        lines.append("  readelf -r binary                          # Relocations")

        lines.append("\n[4] IDA Pro / Ghidra")
        lines.append("  # IDA: F5 = decompile, G = goto address, X = xrefs")
        lines.append("  # IDA: N = rename, ; = comment, Tab = toggle graph/text")
        lines.append("  # Ghidra: free alternative to IDA")
        lines.append("  #   File -> Import File -> Auto Analysis")
        lines.append("  #   Window -> Decompile for pseudocode")

        lines.append("\n[5] Radare2")
        lines.append("  r2 ./binary                                # Open")
        lines.append("  [0x00]> aaa                                # Auto analysis")
        lines.append("  [0x00]> afl                                # List functions")
        lines.append("  [0x00]> pdf @main                          # Disassemble main")
        lines.append("  [0x00]> VV @main                           # Visual graph mode")
        lines.append("  [0x00]> iz                                 # List strings")
        lines.append("  [0x00]> iE                                 # Exports")
        lines.append("  [0x00]> ii                                 # Imports")

        lines.append("\n[6] ltrace / strace")
        lines.append("  ltrace ./binary                            # Library call trace")
        lines.append("  strace ./binary                            # System call trace")
        lines.append("  strace -f -e trace=read,write ./binary     # Filter syscalls")

        lines.append("\n[7] ROPgadget / ropper / one_gadget")
        lines.append("  ROPgadget --binary binary                  # Find all gadgets")
        lines.append("  ROPgadget --binary binary --ropchain       # Auto ROP chain")
        lines.append("  ROPgadget --binary binary | grep 'pop rdi' # Specific gadget")
        lines.append("  ropper -f binary --search 'pop rdi'        # Alternative tool")
        lines.append("  one_gadget libc.so.6                       # Find one-shot gadgets")

        lines.append("\n[8] Python decompilation")
        lines.append("  uncompyle6 file.pyc                        # Python 2.7-3.8")
        lines.append("  decompile3 file.pyc                        # Python 3.9+")
        lines.append("  pycdc file.pyc                             # Decompyle++ (all versions)")
        lines.append("  python -m dis file.pyc                     # Bytecode disassembly")
        lines.append("  python -m py_compile file.py               # Compile to .pyc")

        lines.append("\n[9] Android reverse")
        lines.append("  apktool d app.apk                          # Decompile APK (smali)")
        lines.append("  jadx-gui app.apk                           # Decompile to Java (GUI)")
        lines.append("  jadx app.apk -d output/                    # Decompile to Java (CLI)")
        lines.append("  dex2jar app.apk                            # DEX to JAR")
        lines.append("  jd-gui classes.jar                         # View JAR source")
        lines.append("  adb shell                                  # Device shell")
        lines.append("  frida -U -f com.app -l hook.js             # Frida hooking")

        lines.append("\n[10] .NET / Java reverse")
        lines.append("  dnSpy file.exe                             # .NET decompiler (GUI)")
        lines.append("  ilspy file.dll                             # .NET decompiler (CLI)")
        lines.append("  javap -c -p Class.class                    # Java bytecode")
        lines.append("  cfr Class.class                            # Java decompiler")
        lines.append("  procyon Class.class                        # Java decompiler")

        lines.append("\n[11] Packing / Unpacking")
        lines.append("  upx -d packed.exe                          # UPX unpack")
        lines.append("  file binary                                # Detect packer hints")
        lines.append("  DIE (Detect It Easy)                       # Packer/compiler detection GUI")
        lines.append("  strings binary | grep -i upx               # Check for UPX signature")

        lines.append("\n[12] Useful one-liners")
        lines.append("  checksec --file=binary                     # Security features")
        lines.append("  ldd binary                                 # Linked libraries")
        lines.append("  nm binary                                  # Symbol names")
        lines.append("  file binary                                # File type info")
        lines.append("  rabin2 -I binary                           # Binary info (r2)")
        lines.append("  rabin2 -z binary                           # Strings (r2)")
